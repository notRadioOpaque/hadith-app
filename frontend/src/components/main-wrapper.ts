...
function createHadithContentModal() {
...
function createContent() {
  ...
  content.className =
    "w-[100%] h-[100%]  rounded-lg flex items-center justify-center  text-center flex-col bg-[#242124] p-20 absolute";
  closeBtnDiv.className =
    "flex justify-end items-center absolute top-4 right-4";
  innerContentDiv.className =
  ...
  [contentTitle, contentSubtitle].forEach((child) => {
    innerContentDiv.appendChild(child);
  });

  render([closeBtnDiv, innerContentDiv], content);
